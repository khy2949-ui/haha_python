...
while True:
    ...
    if menu == "1":
        ...
        if not found:   # 과일데이터에서 판매할 과일이 없는 경우    - not 연산자에 의해 거짓 <-> 참
            print("해당 과일이 없습니다.")
        ...
        # 오늘의 추천 과일 - 랜덤
        # import random - randint(1,10): 1~10 사이 정수, randrange(1,10) : 1~9 사이 정수
        # choice(리스트) : 리스트 내분의 값중 하나 선택
        # choice(리스트, k=3) : 리스트 안에서 3개를 선택(중복허용)
        # sample(리스트, 4) : 리스트 안에 값 중 4개를 선택 (중복 없음)
        # shuffle(리스트) : 리스트 안에 값의 순서를 랜덤하게 섞음
